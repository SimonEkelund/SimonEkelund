# Simon Ekelund

Full-stack developer in Helsingborg, Sweden. Diploma in .NET development from EC Utbildning, currently reading Computer Science at Malmö University.

I build systems that other people actually use — and I like the part of the job where you have to sit down with the person whose problem it is.

### Things I've built

- **[BuffBar](https://github.com/SimonEkelund/BuffBar)** — a consumable tracker for WoW Classic, [published on CurseForge](https://www.curseforge.com/wow/addons/buffbar) and maintained across several releases.
- **SPG Metall** — a full site and product-catalogue rebuild for an industrial manufacturer. [Live preview](https://metalworks-navigator.vercel.app).
- **Object detection on live video** — a YOLOv8 model trained from a dataset I collected and annotated myself, reaching 90.1 % mAP50, driving a real-time control loop.
- **[simonekelund.se](https://simonekelund.se)** — my portfolio. Next.js, React, a bit of WebGL.

### Working with

`C#` `.NET` `TypeScript` `React` `Next.js` `Python` `Java` `Flutter` `SQL` `Supabase`

Optimizely and Umbraco from a year at a CMS consultancy. YOLOv8 and OpenCV from training models end to end rather than calling someone else's. Ollama when inference needs to stay on the machine.

### Reach me

[simonekelund.se](https://simonekelund.se) · simonekelund.dev@gmail.com

*A fair amount of my work sits in private repos. Happy to walk through any of it.*
